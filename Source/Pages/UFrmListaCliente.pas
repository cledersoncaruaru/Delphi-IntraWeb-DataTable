...
procedure TFrmListaCliente.BTNDELETEAsyncClick(Sender: TObject;
 ...
 begin
  ...
  if Delete_Pessoa (MinhaChavePrimaria, vResult)then begin
    ...
    WebApplication.CallBackResponse.AddJavaScriptToExecute(swalSuccess('CONFIRMAÇÃO',vResult));
    WebApplication.CallBackResponse.AddJavaScriptToExecuteAsCDATA('$(''#Lista'').dataTable().api().ajax.reload();');

  end else
    WebApplication.CallBackResponse.AddJavaScriptToExecute(swalError('ATENÇÃO',vResult))
...
begin
  ...
  Result  :=False;
  vResult := 'Prezado usuário ocorreu algo problema na exclusão de seu registro.';
...
begin
  ...
  try
    ...
    // aqui é feito para o Firedac Paginar
...
begin
  ...
  if (StrToIntDef(EventParams.Values['Delete'],0) > 0)then begin
      ...
      WebApplication.CallBackResponse.AddJavaScriptToExecute(swalConfirm( 'EXCLUSÃO DE REGISTRO',
